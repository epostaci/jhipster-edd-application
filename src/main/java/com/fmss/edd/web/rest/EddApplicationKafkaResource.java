...
import com.fmss.edd.config.KafkaSseConsumer;
import com.fmss.edd.config.KafkaSseProducer;
...
@RestController
@RequestMapping("/api/edd-application-kafka")
public class EddApplicationKafkaResource {

    private final Logger log = LoggerFactory.getLogger(EddApplicationKafkaResource.class);

    private final MessageChannel output;
    private Sinks.Many<Message<String>> sink = Sinks.many().unicast().onBackpressureBuffer();

    public EddApplicationKafkaResource(@Qualifier(KafkaSseProducer.CHANNELNAME) MessageChannel output) {
        this.output = output;
    }
